...
\hypersetup{
	pdftitle={Tutte evaluations on metric graphs and rank-generating power series},
}
...
% Tutte power series 
attached to any metric graph with arbitrary positive real edge lengths, 
...
evaluation of the rank-generating power series
...
The evaluations give rise to a large family of continuous functions generalizing the one that sends a metric graph to the volume of its graph Jacobian.
%We study how rank-generating power series evaluations are
...
introduced by Tutte~\cite{Tut}.
Many important graph invariants arise as evaluations of 
the Tutte polynomial %$T(G; x,y)$ 
at specific integer parameters $x$, $y$.
% One particular evaluation of the Tutte polynomial
In particular, $T(G; 1, 1)$ is the number of spanning trees of $G$.
...
This motivates the question: can other Tutte polynomial evaluations be generalized to arbitrary metric graphs?
Other Tutte polynomial evaluations include counts of 
connected subgraphs, acyclic subgraphs,
acyclic orientations, and totally cyclic orientations.
In this paper, we show that the function sending a graph $G$ to its Tutte polynomial evaluation $T(G; x, y)$ can be extended to a continuous function on the space of metric graphs, whenever $x > 0$.
This is achieved by defining a rank-generating power series on a metric graph,
which generalizes the usual one on a graph.

As a consequence, when $x > 0$, our definition of Tutte evaluation at a chosen $(x, y)$ gives a continuous function on the space of metric graphs of fixed genus.
\begin{thm}
...
\end{thm}
\todo[inline]{fix theorem statement}
...
We define the {\em rank-generating power series}
...
If all edges in $\Gamma$ have unit length, i.e. $\ell(e) = 1$, then \eqref{eq:tutte-power-series} recovers the Tutte polynomial by the substitution $T(G; x, y) = T^+(\Gamma; x - 1, y - 1)$.
...
\begin{thm}[Rank-generating power series]
...
To prove this result, we show that $T^+(\Gamma; u, w)$ satisfies a familiar deletion-contraction identity (Theorem~\ref{thm:deletion-contraction}).
%\todo{move thm statement out of intro}
...
\subsection{Evaluating rank-generating power series}
...
\begin{figure}[h]
\begin{tabular}{ccc}
% inner tabular-environment is for vertically centering figure
\begin{tabular}{c}
	\begin{tikzpicture}
		\draw (0,0) -- (0,1);
...
The identities presented in this paper are essentially in the work of Read and Whitehead~\cite{RW2}, but with the restriction that edge lengths are positive integers. 
In their work, edge lengths are called ``chain lengths'' since they are viewed as a chain of unit-length edges.
...
\todo{elaborate on these references below}

Application: Zeros of Tutte polynomials?
For example, \cite{brown-hickman} and \cite{BHSW} study zeros of chromatic polynomials in a family of graphs consisting of assigning integer-valued edge lengths to an underlying graph model.
...
Merino--Welsh conjecture~\cite{merino-welsh}: for any graph $G$, Merino and Welsh conjectured that $\max(T(G; 2,0), T(G; 0,2)) \geq T(G; 1,1)$.

\subsection{Questions}

Do the coefficients of the rank generating power series $T^+(G; u,w)$ produce interesting metric graph invariants?
Are these coefficients continuous on the space of metric graphs?
...
$T^+(\Gamma; u,w)$ the rank-generating power series of $\Gamma$
...
= \sum_{k \geq 1} \binom{\en}{k} u^{k - 1},
\]
and the binomial coefficient function $n \mapsto \binom{n}{k}$ has a well-defined extension
\begin{equation}
...
\begin{dfn}
Let $\fanalog{1 + u}{\alpha}$ denote the power series
\begin{equation}
	\fanalog{1+u}{\alpha} \coloneq \alpha + \binom{\alpha}{2} u + \binom{\alpha}{3} u^2 + \cdots
	\in \RR[[u]] .
\end{equation}
...
\section{Rank-generating power series}
...
\begin{eg}[Rank-generating power series of a line]
...
\begin{eg}[Rank-generating power series of a circle]
...
\begin{eg}[Rank-generating power series of theta graph]
...
The rank-generating power series $T^+(\Gamma; u,w)$ satisfies a similar deletion-contraction relation.

\begin{thm}[Deletion-contraction relation]\label{thm:deletion-contraction}
% For a metric graph $\Gamma$,
Given a metric graph $\Gamma = (G,\ell)$ and an edge $e \in E(G)$,
which is not a loop,
the rank-generating power series satisfies
\begin{equation}
	T^+(\Gamma; u,w) 
...
In this section we describe how the definition of rank-generating power series $T^+(\Gamma;u,w)$
...
It sufficies to show that the rank-generating power series is invariant 
under an edge subdivision.
Suppose $G = (V,E)$ contains the edge $e_0$, which we subdivide into $e_1 \cup e_2$ to obtain the graph $G'$.
Suppose the subdivided edges $e_1$ and $e_2$ have lengths $a$ and $b$, so that $e_0$ has length $a + b$.
...
while the compound deletion $G' \setminus \{e_1, e_2\}$ is the disjoint union of 
$G \setminus e_0$ and an isolated vertex.
For the mixed deletion-contraction operations, we have
\[
...
It suffices to show that when a graph in a cell of $\Mggraph$ approaches the boundary of its cell, the limiting value of the rank-generating power series agrees with the rank-generating power series of the limiting graph.
In an equation,
\[
...
Observation: for a fixed combinatorial graph $G$, the $(i,j)$-coefficient of the rank-generating power series $T^+(\Gamma; u,w)$ is a polynomial in the edge lengths.

\section*{Acknowledgements}
The author would like to thank 
Will Dana, Leo Jiang, and
David Speyer for helpful discussions.
...
\]
In terms of the Tutte polynomial, this is
\[
...
\subsection{Rank-generating power series coefficients}
...
\subsection{Rank-generating power series as real-valued function}
Given real parameters $x,y$ with $x > 0$,
let 
\begin{equation}
\label{eq:tutte-metric-graph}
	T(\Gamma; x,y) = \sum_{A \subset E(G)} \left( \prod_{e \in A} \analog{x}{\ell(e)} \right)
...
%Our next result concerns the convergence of the rank-generating power series of a fixed metric graph $\Gamma$.
It is straightforward to verify that the rank-generating power series $T^+(\Gamma;u,w)$ converges to a real value when $|u|<1$.
...
%\begin{eg}[Rank-generating power series of $K_4$]
